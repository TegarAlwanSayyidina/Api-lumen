...
class LendingController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        $totallending = $delete['totallending'];
        $delete ->delete();

        $datastock = StuffStock::where('stuff_id', $delete['$stuff_id'])->first();
        $totalavailable = (int)$datastock['total_available']+(int)$delete['total_stuff'];
        $updateTotalLending = StuffStock::where('stuff_id',$delete['stuff_id'])->update(['total_available' => $totalavailable]);

        if ($updateTotalLending){
            $updatedStuffidTotalLending = Lending::where('id', $delete['stuff_id'])->with('Lending', 'stuffStock')->first();
            
            $delete->delete();
            return ApiFormatter::sendResponse(200, 'succes','Berhasil hapus data');
        }
        }catch (\Exception $err) {
            return ApiFormatter::sendResponse(400, 'bad request', $err->getMessage());
        }
